...
int main()
{
    ...
    {
        1.0e-12, 1.0e-9, 1.0e-6, 1.0e-3, 1.0, 10.0,
        100.0, 1.0e3, 1.0e4, 1.0e6, 1.0e8, 1.0e10
    };
    ...
    for (const Real wall_reynolds : wall_reynolds_values)
    {
        ...
        const Real reconstructed = result.u_plus * result.y_plus;
        const Real relative_error =
            std::fabs(reconstructed - wall_reynolds) / wall_reynolds;
