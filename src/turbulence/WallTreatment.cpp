...
// CBS3D++_SI
//
// Continuous Spalding wall-law kernel for the future SA wall treatment.
//=============================================================================
...
namespace cbs
{
    namespace turbulence
    {
        namespace
        {
            ...
            //
            // without subtractive cancellation when x is small. Only orders 3
            // and 4 are needed by the Spalding relation and its derivative.
            ...
            const WallTreatmentOptions& options)
        {
            ...
            //     Re_y = U_t y / nu = u+ y+
            //
            // The Spalding law satisfies y+(u+) >= u+ for u+ >= 0, so
            // F(u+) = u+ y+(u+) - Re_y is monotone and is guaranteed to be
            // bracketed by [0, sqrt(Re_y)].
            ...
            // Start from the logarithmic asymptote when it lies inside the
            // rigorous bracket. This avoids beginning a high-Re wall solve at
            // u+ = O(sqrt(Re_y)), where the exponential part of Spalding's law
            // would make a Newton step needlessly stiff.
                 ...
                 ++iteration)
            {
                ...
                // Safeguarded Newton step. F'(u+) is strictly positive because
                // y+ and dy+/du+ are positive. If round-off ever places the
                // Newton proposal outside the current bracket, fall back to the
                // bisection midpoint and retain guaranteed convergence.
